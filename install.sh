...
if [ -z "$INSTALLED" ]; then
    ...
    if [[ ! -z $YUM ]]; then
        ...
        sudo apt-get update
        sudo apt-get install build-essential autoconf pkg-config php7.1 python-pip jq libpcre3-dev cabal-install zlib1g-dev liblzma-dev cmake git neovim tmux-next xsel clang libclang-dev python3-pip zbar-tools weechat-devel-curses weechat-devel-plugins
    elif [[ ! -z $PACMAN ]]; then
        sudo pacman -S
    else
